...
int parse_node_list (char *line, char ***node_list)
{
    ...
    *node_list = (char **) calloc (num_nodes, sizeof(char *));
    ...
    for (k = 0; k < num_nodes; k++) {
        ...
        if (strstr(node_name_prefix, "server")) {
            sprintf ((*node_list)[k], "server%d", node_ind);
        } else {
            sprintf ((*node_list)[k], "client%d", node_ind);
        }
...
int get_rank (char* server_name)
{
    ...
    strncpy (hostname, server_name, sizeof(hostname));

    config_info.rank = -1;
    for (i = 0; i < num_servers; i++) {
        printf("config_info.servers[i]:%s\n",config_info.servers[i]);
    ...
    for (i = 0; i < num_clients; i++) {
        printf("config_info.clients[i]:%s\n",config_info.clients[i]);
...
int parse_config_file (char *fname,char* server_name)
{
    int ret = 0;
    FILE *fp = NULL;
    char line[128] = {'\0'};
